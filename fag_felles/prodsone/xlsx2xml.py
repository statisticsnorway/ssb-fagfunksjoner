...
def change_input_to_xml(spec_path, spec_filename, input_path, input_filename, output_path):
    """
    Parameters:
    ...
    for x in range(1, NoOfSeries):
        y = x -1
        series = f"sa{x}"
        b2tf = root.findall(f"""./ns:item/[@name="{series}"]/ns:subset/ns:item/ns:ts/ns:metaData/""", namespace)
        b2tgb = b2tf[2]
    ...
    for x in range(1, NoOfSeries):
        y = x -1
        series = f"sa{x}"
        src = root.findall(f"""./ns:item/[@name="{series}"]/ns:subset/ns:item/ns:ts/ns:metaData/""", namespace)
        src2 = src[1]
        new_src = "Xml"
