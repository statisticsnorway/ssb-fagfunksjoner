    ...
    if not os.path.exists(authpath):
        ...
        with open(authpath, 'r') as f:
            ...
            if 'IOM_Prod_Grid1' not in file:
                print('IOM_Prod_Grid1 mangler fra .authinfo, prøv å kjør saspy_session.set_password() på nytt.')
                return
    felles = os.environ['FELLES']
    cfgtype = 'iomlinux'
    return saspy.SASsession(cfgname=cfgtype,
                            cfgfile=f'{felles}/sascfg.py',
                            encoding='latin1')
        ...
        with open(authpath, 'r') as f:
            for line in f:
                ...
                if 'IOM_Prod_Grid1' in line:
                    file_replaced += 'IOM_Prod_Grid1 user ' + brukernavn + ' password ' + password + '\n'
                else:
                    file_replaced += line
...
def sasfile_to_parquet(path: str, out_path: str = "", gzip: bool = False) -> pd.DataFrame:
    ...
    if not out_path:
        ...
        out_path = out_path.parent.joinpath(out_path.stem.split(".")[0]) # Avoid extra file extensions
