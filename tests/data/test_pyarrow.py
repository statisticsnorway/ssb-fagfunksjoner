...
from fagfunksjoner.data.pyarrow import cast_pyarrow_table_schema, restructur_pyarrow_schema
...
def mock_data():
    size = 50
    data = pa.table([
        pa.array(np.random.choice(['2023-01', '2023-02', '2023-03', '2023-04'], size)),#.dictionary_encode(),
        pa.array(np.random.choice(['xyz', 'abc', 'tre', 'dfg'], size)),#.dictionary_encode(),
        pa.array(np.random.choice(['x1', 'x2', 'x3', 'x4'], size)),#.dictionary_encode(),
