...
from fagfunksjoner.prodsone.dynarev import dynarev_uttrekk  # Replace with the actual import path of your function
...
def test_dynarev_uttrekk_invalid_sfu_cols(mock_oracle):
    with patch("builtins.input", return_value="test_db"):
        with pytest.raises(ValueError):
            dynarev_uttrekk(delreg_nr="1", skjema="test", sfu_cols=123)
...
def test_dynarev_uttrekk_exception_handling(mock_oracle):
    mock_oracle.select.side_effect = Exception("Database error")

    with patch("builtins.input", return_value="test_db"):
        result = dynarev_uttrekk(delreg_nr="1", skjema="test")
