...
def choose_value(dataframe, column, value, operator: str):
    ...
    if (operator == '=='):
        if ("NaN" in str(value)) & (len(value) == 1): # Special treatment of missing value when only missing is selected
            display(dataframe.loc[dataframe[column].isna()])
        elif "NaN" in str(value): # Special treatment of missing value when missing and other is selected
            display(dataframe.loc[(dataframe[column].isna()) | (dataframe[column].isin(value))])
        else:  
...
def view_dataframe(dataframe: pd.DataFrame, column: str, operator: str ='==' , unique_limit: int=100):
    ...
    if operator in operator_comparison:
        if np.issubdtype(dataframe[column].dtype, np.number):
            ...
            filter_type_select = widgets.Dropdown(options=operator_comparison, value=operator, description='Operator')
            threshold_slider = slider_widget(min=dataframe[column].min(), 
                                            max=dataframe[column].max(), 
                                            step=1, 
                                            value=dataframe[column].median(),
                                            description='Value')
            return widgets.interactive(choose_value, 
                                       dataframe=widgets.fixed(dataframe), 
                                       column=widgets.fixed(column), 
                                       operator=filter_type_select,
                                       value=threshold_slider)
        else:
            return widgets.HTML(value="<b>Error:</b> Comparison operators are only allowed for numeric columns. Choose == or != instead.")
    elif operator in ['==', '!=']:
        num_unique = dataframe[column].nunique()
        if num_unique > unique_limit:
            return widgets.HTML(value=f"<b>Error:</b> The number of unique values in column '{column}' is {num_unique}, which exceeds the limit of {unique_limit}. You can change the maximum number of unique values with the unique_limit parameter or switch to a comparison operator.")

        filter_type_select = widgets.Dropdown(options=operator_equality, value=operator, description='Operator')
        if np.issubdtype(dataframe[column].dtype, np.number):
            values_unique = dataframe[column].unique()
            values_unique.sort()
            values_widget = [val if not np.isnan(val) else "NaN" for val in values_unique]
        else:    
            values_unique = dataframe[column].fillna('NaN').unique() # When missing values appear in non-numeric columns, we replace it with the text NaN
            values_unique.sort()
            values_widget = values_unique
        values_select = widgets.SelectMultiple(options=values_widget, description='Value(s)', value=[values_widget[0]])
        return widgets.interactive(choose_value, 
                                   dataframe=widgets.fixed(dataframe), 
                                   column=widgets.fixed(column), 
                                   operator=filter_type_select,
        ...
        return widgets.HTML(value="<b>Error:</b> Invalid operator. Use one of the following: '==', '!=', '>=', '>', '<', '<='")
